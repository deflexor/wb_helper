...
import { useSeoClusters } from '@/hooks/useSeoClusters';
...
// Mock keywords for selector (these would come from a separate hook in production)
...
interface ClusterCardProps {
  cluster: import('@/hooks/useSeoClusters').KeywordCluster;
  onMerge: (clusterId: number) => Promise<void>;
    ...
    <Card className="hover:border-primary/50 transition-colors">
      <CardHeader className="pb-3 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="secondary">{cluster.keywordIds.length}</Badge>
        </div>
      </CardHeader>
      <CardContent className="pt-4">
        {/* Keywords Pills - show count since we don't have text */}
        <div className="flex flex-wrap gap-2 mb-4">
          <Badge variant="outline" className="text-xs">
            {cluster.keywordIds.length} {t('seo.clusters.keywords')}
          </Badge>
        </div>
        ...
        <div className="text-sm text-muted-foreground mb-4">
          ...
          {cluster.articleId ?? t('seo.clusters.noArticle')}
        </div>
...
export default function SeoClustersPage() {
  ...
  const { clusters, createCluster, mergeClusters } = useSeoClusters({ marketplace });
  ...
  const handleCreateCluster = useCallback(async () => {
    ...
    setIsCreating(true);
    try {
      // For now, createCluster takes keywordIds and name
      ...
      window.alert(t('seo.clusters.createError'));
    } finally {
      setIsCreating(false);
    }
  }, [selectedKeywords, newClusterName, t, createCluster]);

  const handleMerge = useCallback(async (clusterId: number) => {
    try {
      await mergeClusters([clusterId]);
      window.alert(t('seo.clusters.mergeSuccess'));
    } catch {
      window.alert(t('seo.clusters.mergeError'));
    }
  }, [mergeClusters, t]);

  const handleSplit = useCallback((clusterId: number) => {
    console.error('Split not implemented for cluster:', clusterId);
  }, []);
  ...
  const handleExport = useCallback((clusterId: number) => {
    ...
    const csv = [
      ['Cluster Name', 'Article ID', 'Avg Position', 'Search Volume', 'Created'],
      [
        cluster.name,
        cluster.articleId ?? '',
        cluster.avgPosition.toString(),
        cluster.totalSearchVolume.toString(),
        cluster.createdAt,
      ],
    ]
      .map((row) => row.join(','))
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-muted/30 rounded-lg">
              <div className="text-2xl font-bold">
                {clusters.reduce((sum, c) => sum + c.keywordIds.length, 0)}
              </div>
              <p className="text-sm text-muted-foreground">
            ...
            <div className="text-center p-4 bg-muted/30 rounded-lg">
              <div className="text-2xl font-bold">
                {new Set(clusters.filter((c) => c.articleId).map((c) => c.articleId)).size}
              </div>
              <p className="text-sm text-muted-foreground">
